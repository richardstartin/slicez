...
public enum DataGenerator {
     ...
     * (0x00007f...), so the top 17 bits of every value are zero. In SliceZ's
     * ~value encoding those bits are always 1 → FULL slices with no payload.
     * Function sizes and sample weights both follow a Zipfian distribution so a
     * handful of hot functions receive most of the samples.
     */
    SAMPLED_PCS {
        @Override
        public long[] generate(int count) {
            ...
            for (int k = 0; k < numFunctions; k++) cumWeight[k] /= totalWeight;
