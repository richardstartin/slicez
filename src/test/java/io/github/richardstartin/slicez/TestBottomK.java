...
class TestBottomK {
    ...
    private static long[] reference(long[] data, int k) {
        long[] sorted = Arrays.stream(data)
                .boxed()
                .sorted(Long::compareUnsigned)
                .mapToLong(Long::longValue)
                .toArray();
    ...
    /**
     * Assert that bottom(k) returns the right count and that the values
     * of the returned rows, sorted by unsigned order, equal reference(data, k).
    ...
    void bottomKthValueConsistentWithCountLTE() {
        ...
        for (int k = 1; k <= data.length; k++) {
            ...
            assertTrue(idx.countLessThanOrEqual(threshold) >= k,
                    "LTE count too small at k=" + k);
            assertTrue(idx.countLessThan(threshold) < k,
                    "LT count not below k at k=" + k);
        }
    }
    ...
    void bottomIsMonotonePrefixByValues() {
        ...
        for (int k = 1; k <= data.length; k++) {
            ...
            assertArrayEquals(
                    Arrays.copyOf(all, k),
                    sortedValues(data, rows),
                    "prefix mismatch at k=" + k);
        }
    }
    ...
    void bottomMultiBlockMinInLaterBlock() {
        ...
        var appender = SliceZ.appender();
        for (int i = 1; i < BLOCK_SIZE + 1; i++) appender.add(100 + i);
        appender.add(1L);
        var idx = appender.build();
        // build matching data array for value lookup: rows 0..BLOCK_SIZE-1 have 101..200,
        // row BLOCK_SIZE has value 1
        long[] data = new long[BLOCK_SIZE + 1];
        for (int i = 0; i < BLOCK_SIZE; i++) data[i] = 101 + i;
        data[BLOCK_SIZE] = 1L;
    ...
    void bottomAllValuesInSecondBlockAreSmaller() {
        ...
        for (int i = 0; i < BLOCK_SIZE; i++) data[i] = BLOCK_SIZE + i;
        for (int i = 0; i < BLOCK_SIZE; i++) data[BLOCK_SIZE + i] = i;
        assertBottomK(data, BLOCK_SIZE + 5);
    }
    ...
    void bottomSingleRowInSecondBlock() {
        ...
        long[] data = new long[BLOCK_SIZE + 1];
        for (int i = 0; i < BLOCK_SIZE; i++) data[i] = i + 101;
        data[BLOCK_SIZE] = 1L;
        assertBottomK(data, 1);
        assertBottomK(data, 2);
    }
    ...
    static Stream<Arguments> bottomDistributions() {
        ...
        return Stream.of(
                ...
                Arguments.of(rng.longs(200, 0, 100).toArray(), 10),
                Arguments.of(rng.longs(200, 0, 100).toArray(), 100),
