...
class TestHeap {
    ...
    void minHeapPollEmptiesInAscendingUnsignedOrder() {
        ...
        for (int i = 0; i < unsorted.length; i++) h.add(new Key(unsorted[i], i));
        long prev = 0;
        while (!h.isEmpty()) {
            var out = h.poll();
    ...
    void minHeapUnsignedOrdering() {
        ...
        var out = h.poll(); assertEquals(0L, out.value);
        out = h.poll(); assertEquals(Long.MAX_VALUE, out.value);
        out = h.poll(); assertEquals(Long.MIN_VALUE, out.value);
        out = h.poll(); assertEquals(-1L, out.value);
    }
    ...
    void maxHeapPollEmptiesInDescendingUnsignedOrder() {
        ...
        while (!h.isEmpty()) {
            var out = h.poll();
            assertTrue(Long.compareUnsigned(prev, out.value) >= 0,
                    "expected descending unsigned order, got " + Long.toUnsignedString(prev) + " then " + Long.toUnsignedString(out.value));
            prev = out.value;
        }
    }
    ...
    void maxHeapUnsignedOrdering() {
        // unsigned: 0 < Long.MAX_VALUE < Long.MIN_VALUE < -1L, so max heap polls -1L first
        var h = new Heap<>(Key.class, (l, r) -> Long.compareUnsigned(r.value, l.value), 4);
        h.add(new Key(0L, 0));
        h.add(new Key(Long.MAX_VALUE, 1));
        h.add(new Key(Long.MIN_VALUE, 2));
        ...
        var out = h.poll(); assertEquals(-1L, out.value);
        out = h.poll(); assertEquals(Long.MIN_VALUE, out.value);
        out = h.poll(); assertEquals(Long.MAX_VALUE, out.value);
        out = h.poll(); assertEquals(0L, out.value);
    }
    ...
    void maxHeapSizeDecreasesOnPoll() {
        ...
        h.add(new Key(3, 2));
        assertEquals(3, h.size());
        var out = h.poll(); assertEquals(2, h.size());
        out = h.poll(); assertEquals(1, h.size());
        out = h.poll(); assertEquals(0, h.size());
        assertTrue(h.isEmpty());
    }
    ...
    void minHeapTailStaleAfterEviction() {
        ...
        for (long i = 1; i <= 5; i++) h.add(new Key(i, (int) i));
        assertEquals(5L, h.tail().value);
    ...
    void minHeapRejectsValueLessThanActualMaxAfterStaleEviction() {
        ...
        assertTrue(h.add(new Key(3, 12)),
                "3 is less than the heap's true max (4) and should be accepted");
    }
    ...
    void minHeapBottomKContentsWrongAfterStaleTail() {
        ...
        for (int i = 0; !h.isEmpty(); i++) polled[i] = h.poll().value;
    ...
    void maxHeapTailStaleAfterEviction() {
        ...
        var h = new Heap<>(Key.class, (l, r) -> Long.compareUnsigned(r.value, l.value), 5);
        for (long i = 5; i >= 1; i--) h.add(new Key(i, (int) i));
